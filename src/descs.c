...
bool read_from_descriptor (DESCRIPTOR_DATA * d)
{
    ...
    while (1) {
        ...
        while (TRUE)
        {
                int nRead = read(d->descriptor, bufin, sizeof (d->inbuf) - 10 - iStart);

                bufin[nRead] = 0;

                if (nRead > 0)
                {
                        translate_telopts(d, (unsigned char *) bufin, nRead, (unsigned char *) d->inbuf, iStart); // MTH 1.5
        ...
        for (d = descriptor_list; d != NULL; d = d->next)
            if (d->descriptor == desc)
                if (d->mth)
                {
                    ...
                    if (d->mth->mccp2)
                    {
                        return write_mccp2(d, txt, length); // MTH 1.5
                    }
                    if (write(desc, txt, length) < 0)
                    {
