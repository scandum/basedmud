...
void char_to_room (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex) {
    ...
    if (ch->desc && ch->desc->mth && ch->desc->mth->msdp_data)
    {
            ...
            for (exit = 0 ; exit < 6 ; exit++)
            {
                ...
                if (pexit == NULL || pexit->to_room == NULL || !char_can_see_room (ch, pexit->to_room))
                {
                    continue;
                }
                cat_sprintf(exits, "\001%c\002%d", *door_get(exit)->name, ch->in_room->exit[exit]->to_room->vnum);
            }
            ...
            msdp_update_var_instant(ch->desc, "ROOM", "%c\001%s\002%d\001%s\002%s\001%s\002%s\001%s\002%s\001%s\002%s%c",
                ...
                "TERRAIN", sector_table[ch->in_room->sector_type].name,
                "EXITS", exits,
                MSDP_TABLE_CLOSE);
    }
}
