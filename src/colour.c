...
int colour_puts (CHAR_DATA *ch, bool use_colour, const char *buf_in, char *buf_out, size_t size)
{
    char temp[20000];
...
//    return buf_out - buf_start;
    ...
    substitute_color(buf_start, temp, use_colour ? 4096 : 0); // MTH 1.5

    strcpy(buf_start, temp);
